...
public class MultiChannelTokenStream extends BufferedTokenStream
{
	...
	public void enable(int channel)
	{
		...
		{
			if(channel==existingChannel) return;
		}
		...
		while(i>=0)
		{
			...
			if(token.getChannel()==channel||!matches(token.getChannel(),channels)) --i;
			else break;
		}
		p=i+1;
	}
	...
	public void disable(int channel)
	{
		...
		{
			if(channels[i]!=channel) channels[remainder++]=channels[i];
		}
	...
	protected Token LB(int k)
	{
		...
		int i=p;
		for(int n=1;n<=k;++n) i=previousTokenOnChannel(i-1,channels);
		return i<0?null:tokens.get(i);
	}
	...
	public Token LT(int k)
	{
		...
		for(int n=1;n<k;++n)
		{
			if(sync(i+1)) i=nextTokenOnChannel(i+1,channels);
		}
		return tokens.get(i);
	}
	...
	protected int previousTokenOnChannel(int i,int[] channels)
	{
		...
		{
			var token=tokens.get(i);
			if(token.getType()==Token.EOF||matches(token.getChannel(),channels)) return i;
			--i;
		}
		return i;
	}
	...
	{
		return Arrays.stream(channels).anyMatch(matchChannel->matchChannel==channel);
	}
}